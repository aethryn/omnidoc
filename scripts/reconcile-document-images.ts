...
type StorageObject={path:string;createdAt:number};
  ...
  while(folders.length){
    ...
    for(let offset=0;;offset+=100){
      ...
      for(const entry of data||[]){
        ...
        const timestamp=Date.parse(entry.created_at||entry.updated_at||"");
        if(Number.isFinite(timestamp))found.push({path,createdAt:timestamp});
      }
...
async function main(){
  const [objects,rows]=await Promise.all([listObjects(),prisma.documentImage.findMany({select:{fileName:true}})]);
