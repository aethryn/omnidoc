...
5. Optionally start Redis with `docker run -d --name omnidoc-redis -p 6379:6379 redis:7-alpine` (or `docker start omnidoc-redis` if it already exists). Redis is used for API rate limits and immediate sign-out revocation, not for document-room coordination.
6. Start both services with `pnpm run dev:all`. The WebSocket development command loads `.env.local` automatically.
7. Verify the collaboration server from a second terminal with `curl http://localhost:4000/health`; expect `"ok":true`.
...
The Redis value must be a writable TLS `rediss://` URL copied from Upstash. Add the same value as a server-only production environment variable in Vercel because the Next.js API uses Redis for distributed rate limits and immediate sign-out propagation. Never expose `REDIS_URL` to the browser. Redis does not decide when collaboration starts and is not used for Yjs room fanout. Keep Cloud Run at one maximum instance because each Yjs room exists in one instance's memory and PostgreSQL persistence is not protected against concurrent writers across instances.
...
Private documents use local TipTap state with REST autosave and do not open the Cloud Run WebSocket service. A document with an active share link or accepted collaborator connects directly to the Yjs WebSocket, even when only one participant is currently present. Cloud Run keeps one in-memory Yjs room per document, loads it from `yjsState` (falling back to PostgreSQL document content), persists updates, and performs a final persistence pass when the last socket closes. Yjs awareness supplies live participant presence. There is no Redis presence heartbeat, leader election, promotion, or demotion state machine. With `min-instances=0`, Cloud Run becomes eligible to scale to zero after all sockets close.
