...
export async function GET(request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  const prior=await prisma.documentCollaborators.findUnique({where:{documentId_userId:{documentId:share.documentId,userId:String(userId)}},select:{revokedAt:true}});
  if(prior?.revokedAt&&share.createdAt<=prior.revokedAt)return NextResponse.json({error:"Your access was revoked. Ask the owner for a new invitation."},{status:403});
