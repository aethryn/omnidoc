...
import { ArrowLeftIcon, ArrowSquareOutIcon, CaretDownIcon, CheckCircleIcon, CheckIcon, ChatCircleDotsIcon, ClockCounterClockwiseIcon, CloudCheckIcon, CopyIcon, DotsThreeIcon, FileDocIcon, FileHtmlIcon, FileMdIcon, FilePdfIcon, FilesIcon, GearSixIcon, GlobeHemisphereWestIcon, ImageIcon, ListBulletsIcon, NotePencilIcon, ShareNetworkIcon, SignOutIcon, SparkleIcon, TextAlignLeftIcon, TextBIcon, TextItalicIcon, TextUnderlineIcon, TrashIcon } from "@phosphor-icons/react";
...
export type InitialDocument={id:string;title:string;content:string;yjsState:string|null;yjsEpoch:number;collaborationEligible:boolean;role:string;status:DocumentStatus;allowComments:boolean;updatedAt:string;lastEditedAt:string;publication:PublicationSummary|null;collaborators:Array<{id:string;name:string;avatar:string;role:string}>};
...
type ManagedCollaborator={userId:string;role:string;acceptedAt:string|null;accessExpiresAt:string|null;connectionCount:number;user:{id:string;name:string;email:string;avatar:string|null}};
...
export default function DocumentEditorClient({initialDocument,currentUser}:{initialDocument?:InitialDocument;currentUser:PresenceUser}){
  ...
  const [yjsEpoch,setYjsEpoch]=useState(initialDocument?.yjsEpoch||0);
  ...
  useEffect(()=>{if(!workspaceUI.sheet&&!workspaceUI.modal)return;const closeOnEscape=(event:KeyboardEvent)=>{if(event.key!=="Escape")return;if(workspaceUI.sheet)dispatchWorkspace({type:"close-sheet"});else dispatchWorkspace({type:"close-modal"});};window.addEventListener("keydown",closeOnEscape);return()=>window.removeEventListener("keydown",closeOnEscape);},[workspaceUI.sheet,workspaceUI.modal]);
  ...
  const ensureSaved=useCallback((contentOverride?:string)=>{
    ...
    creationRef.current=fetch("/api/documents",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({title:documentNameRef.current,content:json}),signal:AbortSignal.timeout(15_000)}).then(async(response)=>{if(!response.ok)throw new Error("Could not create document");const created=await response.json();activeIdRef.current=created.id;setActiveId(created.id);setInitialYjsState(created.yjsState);setYjsEpoch(created.yjsEpoch||0);setLastSaved(new Date().toISOString());setDirty(false);localStorage.removeItem(recoveryKey(currentUser.id));setRecoveryDraft(null);router.replace(`/document/${created.id}`);return created.id as string;}).catch(()=>{setSaveError(true);localStorage.setItem(recoveryKey(currentUser.id),json);setRecoveryDraft(json);return null;}).finally(()=>{setIsSaving(false);creationRef.current=null;});
    return creationRef.current;
  },[currentUser.id,router]);

  const contentChanged=useCallback((json:string)=>{contentRef.current=json;setContent(json);setEmbeddedImageUrls(extractDocumentImageUrls(json));setSaveError(false);if(!realtimeEditorActive)setDirty(true);if(publication?.isActive)setHasUnpublishedChanges(true);if(!activeIdRef.current)void ensureSaved(json);},[ensureSaved,publication?.isActive,realtimeEditorActive]);
  ...
  async function save(documentId?:string){
    ...
    if(realtimeEditorActive){const acknowledged=await editorRef.current?.createCheckpoint("Manual checkpoint",documentName);if(acknowledged){setDirty(false);setLastSaved(new Date().toISOString());}return Boolean(acknowledged)&&collaborationStatus!=="error";}
    setIsSaving(true);setSaveError(false);const json=editorRef.current?.getDocumentJSON()||content;
    try {
      const response=await fetch(`/api/documents/${id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({title:documentName,content:json}),signal:AbortSignal.timeout(15_000)});
      ...
      return response.ok;
    } catch { setSaveError(true);localStorage.setItem(`document-${id}-backup`,json); return false; } finally { setIsSaving(false); }
  }
  async function signOut(){
    if(signingOut)return;
    setSigningOut(true);
    try{await fetch("/api/auth/signout",{method:"POST",credentials:"include",cache:"no-store"});window.dispatchEvent(new Event("omnidoc:signed-out"));}finally{router.replace("/");}
  }
  ...
  async function publish(){if(role!=="owner")return;const id=await ensureSaved();if(!id)return;setPublishBusy(true);if(realtimeEditorActive&&collaborationStatus!=="error"){const checkpointed=await editorRef.current?.createCheckpoint("Before publishing",documentName);if(!checkpointed){setPublishBusy(false);toast.error("Save the latest changes before publishing");return;}}else if(!await save(id)){setPublishBusy(false);toast.error("Save the latest changes before publishing");return;}const response=await fetch(`/api/documents/${id}/publication`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({title:documentName})});const data=await response.json().catch(()=>({}));if(response.ok){setPublication({...data,isActive:true,updatedAt:new Date().toISOString()});setHasUnpublishedChanges(false);setDirty(false);toast.success(publication?"Published page updated":"Document published");}else toast.error(data.error||"Document could not be published");setPublishBusy(false);}
  ...
  async function loadShareLinks(){const id=await ensureSaved();if(!id)return null;setShareUrl("");const [response,members]=await Promise.all([fetch(`/api/documents/${id}/share`,{cache:"no-store"}),role==="owner"?fetch(`/api/documents/${id}/collaborators`,{cache:"no-store"}):Promise.resolve(null)]);if(response.ok)setShareLinks(await response.json());if(members?.ok){const data=await members.json();setManagedCollaborators(data.items||[]);}return id;}
  ...
  async function createShare(){if(!activeId)return;setShareBusy(true);if(!realtimeEditorActive&&!await save(activeId)){toast.error("Save the document before sharing");setShareBusy(false);return;}const response=await fetch(`/api/documents/${activeId}/share`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({role:shareRole,expiresInMinutes:Number(expiry),maxUses:maxUses?Number(maxUses):null})});if(response.ok){const data=await response.json();setShareUrl(data.url);setInitialYjsState(data.yjsState);setYjsEpoch(data.yjsEpoch||0);setCollaborationStatus("connecting");setCollaborationEligible(true);const list=await fetch(`/api/documents/${activeId}/share`,{cache:"no-store"});if(list.ok)setShareLinks(await list.json());}else toast.error("Invite link could not be created");setShareBusy(false);}
  ...
  async function removeCollaborator(userId:string){if(!activeId||!window.confirm("Remove this collaborator's access and disconnect their active sessions?"))return;const response=await fetch(`/api/documents/${activeId}/collaborators`,{method:"DELETE",headers:{"Content-Type":"application/json"},body:JSON.stringify({userId})});const data=await response.json().catch(()=>({}));if(!response.ok){toast.error(data.error||"Collaborator could not be removed");return;}setManagedCollaborators((items)=>items.filter((item)=>item.userId!==userId));if(data.collaborationEligible===false){setCollaborationEligible(false);setInitialYjsState(null);setCollaborationStatus("local");setCollaborationState((state)=>({...state,connectivity:"online",pendingLocalChanges:false,syncError:null,retryAvailable:false}));if(typeof data.content==="string"){setContent(data.content);contentRef.current=data.content;}}toast.success("Collaborator removed");}

  const savedLabel=isSaving?"Saving…":saveError?"Save failed":activeId?(!isOnline||realtimeEditorActive&&collaborationState.connectivity==="offline"?"Saved locally":realtimeEditorActive&&collaborationState.connectivity==="connecting"?"Connecting live collaboration…":realtimeEditorActive&&collaborationState.syncError?"Live collaboration unavailable":realtimeEditorActive&&collaborationState.pendingLocalChanges?"Saving…":dirty?"Saving…":realtimeEditorActive?`Live · ${livePresence.length} here`:"All changes saved"):!isOnline?"Saved locally":dirty?"Saving…":lastSaved?"Saved":"Local draft";
  ...
  return <SidebarProvider defaultOpen={false} className="document-sidebar-provider min-h-0">
    <Sidebar side="left" collapsible="offcanvas" className="border-r border-[#e3ddd3] bg-[#fffdf8]">
        ...
        <SidebarGroup>
          ...
          <SidebarGroupContent><SidebarMenu>
            ...
            {!readOnly&&<SidebarMenuItem><details className="group rounded-xl" onToggle={(event)=>{if(event.currentTarget.open)void loadShareLinks();}}><summary className="flex h-11 cursor-pointer list-none items-center gap-2 rounded-xl px-3 text-[13px] hover:bg-sidebar-accent"><ShareNetworkIcon className="text-[#75688a]"/><span className="flex-1">Share</span><CaretDownIcon className="transition-transform group-open:rotate-180"/></summary><div className="mx-2 grid gap-2 rounded-xl border border-[#e3ddd3] bg-white/70 p-3">
              ...
              <Button size="sm" onClick={()=>void createShare()} loading={shareBusy} loadingLabel="Creating…" className="rounded-full bg-[#7140cd] text-white hover:bg-[#6032b8]">Create invite</Button>
            ...
            {role==="owner"&&status==="COMPLETE"&&<SidebarMenuItem><details className="group rounded-xl"><summary className="flex h-11 cursor-pointer list-none items-center gap-2 rounded-xl px-3 text-[13px] hover:bg-sidebar-accent"><GlobeHemisphereWestIcon className="text-[#75688a]"/><span className="flex-1">{publication?.isActive?"Publication":"Publish"}</span><CaretDownIcon className="transition-transform group-open:rotate-180"/></summary><div className="mx-2 grid gap-2 rounded-xl border border-[#e3ddd3] bg-white/70 p-3 text-xs">
              ...
              <Button size="sm" className="rounded-full bg-[#40355f] text-white" onClick={()=>void publish()} loading={publishBusy} loadingLabel="Publishing…" disabled={!hasUnpublishedChanges&&Boolean(publication?.isActive)}>{publication?.isActive?"Republish":"Publish"}</Button>
      ...
      <SidebarFooter className="border-t border-[#e8e2d8] bg-[#fffdf8] p-4 pb-[calc(1rem+var(--safe-area-bottom))]"><div className="flex items-center gap-3"><UserAvatar name={currentUser.name} src={currentUser.avatar} className="grid h-9 w-9 place-items-center overflow-hidden rounded-full bg-[#352d59] text-xs text-white"/><div className="min-w-0 flex-1"><p className="truncate text-xs font-medium">{currentUser.name}</p><p className="text-[10px] text-[#8c847a]">{role==="owner"?"Owner":role}</p></div><button className="grid h-9 w-9 place-items-center rounded-full text-[#756d63] hover:bg-[#f0ebe4]" aria-label="Sign out" title="Sign out" disabled={signingOut} onClick={()=>void signOut()}><SignOutIcon/></button></div></SidebarFooter>
    </Sidebar>
    ...
    <input ref={imageInputRef} className="sr-only" type="file" accept=".jpg,.jpeg,.png,.gif,image/jpeg,image/png,image/gif" onChange={(event)=>{const file=event.target.files?.[0];event.target.value="";if(file)void insertImage(file);}} />
     <header className="workspace-topbar"><div className="workspace-wordmark"><SidebarTrigger className="mobile-document-sidebar-trigger h-9 w-9 rounded-full"/><button className="workspace-logo" onClick={()=>router.push("/dashboard")} aria-label="Back to documents"><OmnidocLogo priority className="workspace-logo-image"/></button><div className="workspace-breadcrumb"><span>Omnidoc</span><span className="workspace-slash">/</span><span className="document-name">{documentName}</span></div></div><div className="topbar-actions">
      <div className={`save-state ${(collaborationState.syncError||saveError)?"is-error":""}`}>{isOnline&&!collaborationState.syncError&&!saveError?<CloudCheckIcon/>:<ClockCounterClockwiseIcon/>}<span className={`save-state__dot ${(dirty||collaborationState.pendingLocalChanges)?"pending":""}`}/>{savedLabel}{saveError&&<button type="button" className="ml-1 rounded border border-[#bdaecb] px-1.5 py-0.5 text-[9px] text-[#694d83] hover:bg-[#eee8f4]" onClick={()=>void save()} aria-label="Retry saving changes">Retry</button>}{collaborationState.retryAvailable&&<button type="button" className="ml-1 rounded border border-[#bdaecb] px-1.5 py-0.5 text-[9px] text-[#694d83] hover:bg-[#eee8f4]" onClick={()=>editorRef.current?.retryPersistence()} aria-label="Retry syncing changes">Retry</button>}</div>
      <Popover open={presenceOpen} onOpenChange={(open)=>{setPresenceOpen(open);if(open)setAiOpen(false);}}><PopoverTrigger asChild><button className="collaborator-tab"><span className="avatar-stack">{livePresence.slice(0,3).map((person,index)=><UserAvatar key={person.id} name={person.name} src={person.avatar} className="mini-avatar" style={{background:person.color||colors[index%colors.length]}}/>)}</span><span>{livePresence.length} here</span></button></PopoverTrigger><PopoverContent align="end" sideOffset={8} className="collaborator-popover"><h3>People in this document</h3>{knownPeople.map((person)=><div className="person-row" key={person.id}><UserAvatar name={person.name} src={person.avatar} className="mini-avatar" style={{background:person.color}}/><div><p>{person.id===currentUser.id?"You":person.name}</p><span>{livePresence.some((live)=>live.id===person.id)?"Active now":person.role||"Collaborator"}</span></div>{livePresence.some((live)=>live.id===person.id)&&<i className="online-dot"/>}</div>)}</PopoverContent></Popover>
