...
import { documentToHtml } from "@/lib/document-content";
import { markdownToPlainText, markdownToTiptap } from "@/lib/markdown-to-tiptap";
import type { AiEditScope, AiEditResult } from "@/lib/ai/edit-operation";
...
export function AIAssistantPanel({ onClose, editorRef, documentId, embeddedImageUrls = [], onOpenSettings, readOnly, selection: preservedSelection }: { onClose:()=>void; editorRef:RefObject<EditorHandle | null>; documentId?:string; embeddedImageUrls?:string[]; onOpenSettings:()=>void; readOnly?:boolean; selection?: EditorSelection|null }) {
  ...
  const [scope,setScope]=useState<AiEditScope>("auto");
  ...
  async function submit(event?:FormEvent) {
    ...
    if(!editor)return;
    const snapshot=editor.getEditSnapshot();
    const selection=preservedSelection||snapshot.selection;
    ...
    let complete="";
    try{
      const response=await fetch("/api/ai/edit",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({documentId,instruction,scope,selection:selection.text?selection:null,caret:snapshot.caret,blocks:snapshot.blocks.map(({id,index,text})=>({id,index,text})),imageIds:selectedImageIds}),signal:controller.signal});
      ...
      while(true){const {done,value}=await reader.read();if(done)break;buffer+=decoder.decode(value,{stream:true});const chunks=buffer.split("\n\n");buffer=chunks.pop()??"";for(const chunk of chunks){const eventType=chunk.match(/^event:\s*(.+)$/m)?.[1];const raw=chunk.match(/^data:\s*(.+)$/m)?.[1];if(!raw)continue;const data=JSON.parse(raw);if(eventType==="suggestion"){const result=data as AiEditResult;let from=selection.from;let to=selection.to;let originalText=selection.text;if(result.operation==="replace-blocks"&&result.startBlockId&&result.endBlockId){const start=snapshot.blocks.find((block)=>block.id===result.startBlockId);const end=snapshot.blocks.find((block)=>block.id===result.endBlockId);if(!start||!end)throw new Error("Omni could not resolve that section");from=start.from;to=end.to;originalText=snapshot.blocks.slice(start.index,end.index+1).map((block)=>block.text).join("\n");}else if(result.operation==="insert-at-caret"){from=snapshot.caret;to=snapshot.caret;originalText="";}else if(result.operation==="replace-document"){from=0;to=snapshot.blocks.at(-1)?.to||0;originalText=snapshot.blocks.map((block)=>block.text).join("\n");}const next={id,text:markdownToPlainText(result.markdown),markdown:result.markdown,from,to,originalText,operation:result.operation};setSuggestion(next);editor.previewSuggestion(next);}if(eventType==="error")throw new Error(data.error||"Provider request failed");}}
  ...
  async function accept(){
    ...
    setApplying(true);
    try {
      if(suggestion.operation==="replace-document") {
        const checkpointed=await editorRef.current?.createCheckpoint("Before Omni full-document rewrite");
        if(!checkpointed){setError("Omni could not create a safety checkpoint. Save the document and try again.");return;}
      }
      const didApply=editorRef.current?.applySuggestion(suggestion);
      if(!didApply){setError("The target text changed while Omni was writing. Copy the suggestion or try again.");return;}
      setApplied(true);
  ...
  function removeSelected(){const selection=preservedSelection||editorRef.current?.getSelection();if(!selection?.text||readOnly)return;const next:EditorSuggestion={id:crypto.randomUUID(),text:"",markdown:"",from:selection.from,to:selection.to,originalText:selection.text,kind:"delete",operation:"replace-selection"};setRequest("Remove selected text");setError(null);setApplied(false);setSuggestion(next);editorRef.current?.previewSuggestion(next);}

  const previewHtml=suggestion?.markdown?documentToHtml(JSON.stringify(markdownToTiptap(suggestion.markdown))):"";
  ...
  return <motion.aside initial={{x:28,opacity:0}} animate={{x:0,opacity:1}} exit={{x:28,opacity:0}} transition={{duration:.22}} className="ai-panel" aria-label="Omni AI assistant">
    ...
    <div className="ai-panel__conversation">
      ...
      {(suggestion||loading||error)&&<div className="ai-response"><div className="ai-response__label"><SparkleIcon weight="fill"/> {loading?"Writing suggestion…":error&&!suggestion?"Omni needs attention":suggestion?.operation==="replace-document"?"Replace entire document":"Suggested change"}</div>{suggestion?.kind==="delete"?<p>The selected text will be removed after you accept.</p>:previewHtml?<div className="ai-markdown-preview" dangerouslySetInnerHTML={{__html:previewHtml}}/>:suggestion?.text&&<p>{suggestion.text}</p>}{error&&<p className="!font-[var(--font-inter)] !text-xs !leading-5 !text-[#a63f36]">{error}</p>}<div className="ai-response__actions">{needsSettings&&<Button variant="outline" size="sm" onClick={onOpenSettings}>Open AI settings</Button>}{applied?<span className="ai-applied"><CheckIcon weight="bold"/>Added to document</span>:suggestion&&<><Button variant="ghost" size="sm" disabled={!suggestion.text||applying} onClick={()=>navigator.clipboard?.writeText(suggestion.text)}><CopyIcon/>Copy</Button><Button variant="outline" size="sm" onClick={dismiss} disabled={applying}>Dismiss</Button><Button size="sm" onClick={()=>void accept()} loading={applying} loadingLabel="Applying…" disabled={loading} className="bg-[#1c1c1e] text-white hover:bg-[#2c2c34]">{suggestion.operation==="replace-document"?"Replace document":"Accept change"}</Button></>}</div></div>}
    </div>
    <form className="ai-composer" onSubmit={submit}><label className="ai-scope-label">Target<select value={scope} disabled={loading||readOnly} onChange={(event)=>setScope(event.target.value as AiEditScope)}><option value="auto">Auto</option><option value="selection" disabled={!(preservedSelection||editorRef.current?.getSelection())?.text}>Selected text</option><option value="document">Entire document</option></select></label><Textarea value={prompt} disabled={readOnly||loading} onChange={(event)=>setPrompt(event.target.value)} onKeyDown={(event)=>{if(event.key==="Enter"&&!event.shiftKey){event.preventDefault();submit();}}} placeholder={readOnly?"Viewers can’t edit this document":"Ask Omni to change this document…"}/><div className="ai-composer__footer"><span>{loading?"Writing structured edit…":"Enter to submit"}</span><Button type="submit" size="icon" disabled={loading||readOnly||!prompt.trim()} className="h-8 w-8 rounded-full bg-[#1c1c1e] text-white"><ArrowUpIcon weight="bold"/></Button></div></form>
  </motion.aside>;
}
